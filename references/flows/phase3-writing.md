...
**关键提醒（每章完成后必须遵守）**：
> 本章已完成。立即读取 `02-写作计划.json`，认领下一个 pending 章节，开始下一章创作。不要再次向用户做逐章确认，不要停下来。你必须把所有章节创作完成才能与用户报告。
...
1. 计算批次分配:
   ...
   - 批次间不重叠
   - 示例: 30 章 → 5 批 × 6 章
2. 为每个批次派生子 Agent（仅在当前环境支持代理并行且用户明确要求时使用）:
...
此模式仅作为上游仓库的设计记录保留。当前本地 Codex skill 中不要选择或执行该模式。
